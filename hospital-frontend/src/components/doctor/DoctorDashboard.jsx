...
const DoctorDashboard = () => {
  ...
  const getStatusColor = (status) => {
    const colors = {
      ...
      'in-progress': 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300',
  ...
  const getConditionColor = (status) => {
    const colors = {
      ...
      'Recovering': 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300'
    };
    return colors[status] || 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300';
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Welcome back, Dr. Smith! 👋</h1>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link to="/doctor/appointments" className="flex items-center gap-4 p-4 border-2 border-blue-200 dark:border-blue-700 rounded-lg hover:border-blue-500 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all group">
            <div className="p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg group-hover:bg-blue-500 transition-colors">
              <Calendar className="w-6 h-6 text-blue-600 dark:text-blue-400 group-hover:text-white" />
            </div>
            <div>
          ...
          <Link to="/doctor/prescriptions" className="flex items-center gap-4 p-4 border-2 border-green-200 dark:border-green-700 rounded-lg hover:border-green-500 dark:hover:border-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 transition-all group">
            <div className="p-3 bg-green-100 dark:bg-green-900/30 rounded-lg group-hover:bg-green-500 transition-colors">
              <Pill className="w-6 h-6 text-green-600 dark:text-green-400 group-hover:text-white" />
            </div>
            <div>
              <p className="font-semibold text-gray-900 dark:text-white">Add Prescription</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Write new prescription</p>
            </div>
          </Link>

          <Link to="/doctor/records" className="flex items-center gap-4 p-4 border-2 border-purple-200 dark:border-purple-700 rounded-lg hover:border-purple-500 dark:hover:border-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-all group">
            <div className="p-3 bg-purple-100 dark:bg-purple-900/30 rounded-lg group-hover:bg-purple-500 transition-colors">
              <ClipboardList className="w-6 h-6 text-purple-600 dark:text-purple-400 group-hover:text-white" />
            </div>
            <div>
              <p className="font-semibold text-gray-900 dark:text-white">Request Lab Test</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Order diagnostic tests</p>
            </div>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/doctor/appointments" className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 text-sm font-medium">
              View All →
            </Link>
          </div>
          ...
          <div className="space-y-4">
            {todaysAppointments.map((appointment) => (
              <div key={appointment.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors border border-gray-200 dark:border-gray-600">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">{appointment.reason} • Age {appointment.age}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Weekly Patient Visits</h2>
              <TrendingUp className="w-5 h-5 text-green-500" />
            </div>
            <div className="space-y-3">
              {weeklyData.map((data, index) => (
                <div key={index} className="flex items-center gap-3">
                  <span className="text-xs font-medium text-gray-600 dark:text-gray-400 w-8">{data.day}</span>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-green-600 dark:text-green-400">94.5%</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/doctor/patients" className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 text-sm font-medium">
                View All →
              </Link>
            </div>
            <div className="space-y-4">
              {recentPatients.map((patient) => (
                <div key={patient.id} className="flex items-start space-x-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors border border-gray-200 dark:border-gray-600">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-300 mb-1">{patient.condition}</p>
              ...
              {notifications.map((notification) => (
                <div key={notification.id} className={`flex items-start gap-3 p-3 rounded-lg ${
                  ...
                  <div className={`p-2 rounded-lg ${
                    notification.urgent ? 'bg-red-100 dark:bg-red-900/30' : 'bg-blue-100 dark:bg-blue-900/30'
                  }`}>
                    {notification.type === 'patient' && <Users className="w-4 h-4 text-blue-600 dark:text-blue-400" />}
                    {notification.type === 'lab' && <ClipboardList className="w-4 h-4 text-red-600 dark:text-red-400" />}
                    {notification.type === 'appointment' && <Calendar className="w-4 h-4 text-blue-600 dark:text-blue-400" />}
                  </div>
                  <div className="flex-1 min-w-0">
